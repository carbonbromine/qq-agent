...
子开关“主动好友候选”允许 Agent 基于真实互动提交候选，由指定管理员通过控制台或
私聊命令审批，并提供最低互动数、待审批上限与默认 30 天冷却。当前 SnowLuma
1.14.15 没有主动发起好友申请的 action，因此批准结果会明确标记为“待手动执行”，
收到 `friend_add` 事件或重新索引到好友关系后再闭环为“已成为好友”，不会伪报成功。
