...
- prompt-mode 主动好友候选仍会正常创建并以 `pending` 状态持久化；不会因为缺少管理员 QQ 丢掉候选，只会在该候选的 `notifyError` 中记录通知不可用；
- triggered/manual 好友评估同样可以完成并保留候选，管理员只决定 QQ 通知与私聊审批入口是否可用；
- 入站好友请求仍可持久化；
- Incident 数据库、异常采集和会话控制照常启动；
- 只跳过需要管理员参与的 QQ 通知/私聊审批边缘动作。管理员之后配置完成后，可继续从控制台处理已持久化的候选和请求。
